...
// 必须走同源 Next.js API 代理 /api/auth/children-create，由 Next.js 进程内转发到 auth-service。
// 详见 apps/travel-guide/src/app/api/auth/children-create/route.ts
...
export async function createChildFromClient(
  ...
  const res = await authedFetch('/api/auth/children-create', {
    method: 'POST',
